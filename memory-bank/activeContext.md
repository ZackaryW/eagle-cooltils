...
## Current Focus
**Library UUID Config Mode** - Added persistent UUID-based library identification for config that survives library moves/renames.

## Recent Changes
- Added `useLibraryUuid` option to `EagleUserConfig` for persistent library identification
  - Stores UUID v4 in `cooler-uuid.json` in the library root folder
  - Auto-generates UUID on first access if file doesn't exist
  - Config persists even if library folder is moved or renamed
  - New factory functions: `createLibraryUuidConfig()`, `createLibraryUuidPluginConfig()`
  - Exported helpers: `getOrCreateLibraryUuid()`, `readLibraryUuid()`, `writeLibraryUuid()`
  - `getResolvedKey()` is now async (UUID mode requires file system access)
